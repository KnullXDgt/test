    ...
    if state then
        ...
        SupportState.autoEquipRodConn = Data.Player:OnChange("EquippedId", function(value)
            if not value or value == "" then
                ...
                task.wait(0.2)
                -- Re-check setelah delay; SellHold mungkin sudah aktif oleh quest op.
...
Navigation.pauseQuestForEvent = function(state)
    ...
    if state then
        ...
        if S.Quest then
            for _, job in ipairs({"Artifact","DeepSea","Element","Diamond"}) do
                if Runtime.Quest.Enabled[job] and Runtime.Quest.Start then
...
do
    ...
    -- ====== QUEST HELPERS (refactored: independent per-feature loops) ======
    ...
    -- withSellHold: guaranteed release via pcall wrapper, flush sell setelah selesai
    ...
    -- equipRodCanonical: equip progression rod agresif dengan verified equip
    ...
    S.Quest.placeStateItem = function(job, remote, stateKey, typeName)
        ...
        S.Quest.withSellHold(function()
            for attempt = 1, 3 do
                ...
                -- cek dulu kalau sudah aktif sebelum fire
    ...
    S.Quest.exchangeItem = function(job, questName, objectiveId, args)
        ...
        S.Quest.withSellHold(function()
            for attempt = 1, 3 do
                if Runtime.Quest.Enabled[job] ~= true then break end
                -- cek dulu kalau sudah progressed (double-fire guard)
    ...
    -- createTranscended: equip secret fish + InvokeServer + waitJob consumed
    ...
    -- findPressureFish: lookup via category "Items" (sesuai probe4 controller)
    ...
    -- Smart hotbar: evict slot ujung kalau penuh, restore setelah selesai
    S.Quest.placePressureFishEntry = function(job, definition)
        local ok = false
        S.Quest.withSellHold(function()
            for attempt = 1, 3 do
                if Runtime.Quest.Enabled[job] ~= true then break end
                -- cek dulu apakah plate sudah aktif
                ...
                if not entry then break end -- ikan tidak ada di inventory
    ...
    -- ====== INDEPENDENT FEATURE RUNNERS ======

    local function startJobThread(job, fn)
    ...
    -- ARTIFACT: teleport ke tiap CFrame, place lever jika item ada, retry 3x
    local function runArtifact()
        while Runtime.Quest.Enabled.Artifact == true do
            ...
            for _, definition in ipairs(S.Quest.Artifacts) do
                if levers[definition.Type] ~= true then
                    ...
                    -- break: 1 artifact per iteration, re-cek levers di loop berikutnya
                    break
                end
            end
            if allDone then
    ...
    local function runDeepSea()
        ...
        while Runtime.Quest.Enabled.DeepSea == true do
            ...
            -- objective 4 (1M coins): diam di posisi terakhir
            task.wait(0.5)
        end
    end

    -- ELEMENT: silent wait eligibility, activate di AJ, loop per objective
    local function runElement()
        -- Silent eligibility: tunggu Ghostfinn Rod via Replion
        ...
        while Runtime.Quest.Enabled.Element == true do
            ...
            elseif S.Quest.progress("Element Quest", 3, 1) < 1 then
                if S.Quest.get("UnlockedTemple") == true then
                    ...
                    -- STAY: temple belum unlock, poll tiap 1s via loop
                    -- (Replion OnChange("UnlockedTemple") akan trigger RefreshPanels
                    --  dan loop iteration berikutnya akan cek ulang)
                end
    ...
    local function runDiamond()
        -- Silent eligibility: tunggu Element Rod / Key / progress
        ...
        while Runtime.Quest.Enabled.Diamond == true do
            ...
            -- obj6 (1000 perfect): diam di posisi terakhir, panel via Replion
            task.wait(0.4)
        end
    end

    -- CRYSTALLINE (pure event-driven, bukan loop thread)
    -- Guard double-attempt: CrystallineBusy[fishName] = true saat attempt running
    local CrystallineBusy = {}
    ...
    Runtime.Quest.OnFishCaught = function(fishName, metadata)
        ...
        if CrystallineBusy[fishName] then return end
        CrystallineBusy[fishName] = true
        task.spawn(function()
            pcall(function()
                S.Quest.placePressureFishEntry("Crystalline", targetDef)
            end)
            CrystallineBusy[fishName] = nil
    ...
    Runtime.Quest.Start = function(job)
        ...
        if job == "Artifact" then
            startJobThread("Artifact", runArtifact)
        elseif job == "DeepSea" then
            startJobThread("DeepSea", runDeepSea)
        elseif job == "Element" then
            startJobThread("Element", runElement)
        elseif job == "Diamond" then
            startJobThread("Diamond", runDiamond)
        end
    ...
    Runtime.Quest.Stop = function(job)
        ...
        -- jadi withSellHold tidak bisa release sendiri kalau thread di-cancel
        Runtime.Quest.SellHold = false
        task.defer(function()
            if Runtime.Sell.Pending and Runtime.Fishing.Phase == "Idle" then
                Runtime.Sell.Flush()
